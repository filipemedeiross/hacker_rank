...
char**** get_document(char* text) {
    ...
    for (int i = 0; text[i]; i++) {
        ...
        if (ch == ' ' || ch == '.') {
            document[p][s] = alloc_word(document[p][s], word, w);
            pw = 0;

            if (ch == ' ')
                w++;
            else if (text[i + 1] != '\n') {
                document[p] = (char***) realloc(document[p], (++s + 1) * sizeof(char**));
                w = 0;
            }
        }
        else if (ch == '\n') {
